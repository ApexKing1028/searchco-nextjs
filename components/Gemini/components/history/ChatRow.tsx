...
const ChatRow: React.FC<Props> = ({ id, onClickChat }) => {
    ...
    const [data] = useCollection(
        user && query(
            collection(db, 'history', user?.email!, 'gemini', id, 'messages'),
    ...
    const removeChat = async () => {
        await deleteDoc(doc(db, 'history', user?.email!, 'gemini', id))
        // @ts-ignored
        onClickChat()
    ...
    return (
        <>
            <Link href={`/gemini/chat/${id}`} onClick={onClickChat} className={`chatRow justify-center hover:bg-gray-500/10 ${active && 'bg-gray-500/10 dark:bg-gray-700/50'}`}>
